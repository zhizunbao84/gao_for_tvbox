...
var rule = {
    ...
    lazy:`js:
        ...
        let result = {};
        let iurl = 'https://api.bilibili.com/x/player/wbi/playurl?avid=' + ids[0] + '&cid=' + ids[1] + '&fnval=4048&fnver=0&fourk=1&otype=json&qn=0&try_look=1&wts=' + Math.floor(new Date().getTime() / 1000);
        log('yingshi aid:  ' + ids[0]);
        let html = request(iurl);
        let jRoot = JSON.parse(html);
        let jo = jRoot.data;
        let accept_desc = jo.accept_description;
        let accept_quality = jo.accept_quality;
        let videos = jo.dash.video;
        let myurls = [];
        let video_ids = [];
        ...
        let video_ids2 = Array.from(new Set(video_ids));
        video_ids2.forEach(function(id) {
            var video_url = getProxyUrl() + '&aid=' + ids[0] + '&cid=' + ids[1] + '&qn=' + id + '&type=mpd';
            myurls.push(accept_desc[accept_quality.indexOf(id)]);
            myurls.push(video_url);
        });
        result.parse = 0;
        result.playUrl = '';
        result.url = myurls;
    ...
    var mytype = input.type;
    if (mytype == 'mpd') {
        var myaid = input.aid;
        log('yingshi myaid:   ' + myaid);
        var mycid = input.cid;
        log('yingshi mycid:   ' + mycid);
        var myqn = input.qn;
        let iurl = 'https://api.bilibili.com/x/player/playurl?avid=' + myaid + '&cid=' + mycid + '&fnval=4048&fnver=0&fourk=1&otype=json&qn='+ myqn +'&try_look=1&wts=' + Math.floor(new Date().getTime() / 1000);
        let html = request(iurl);
        log('yingshi iurl:   ' + iurl);
        let jRoot = JSON.parse(html);
        let jo = jRoot.data; 
        let videos = jo.dash.video;
        log('yingshi videos:  ' + videos);
        let audios = jo.dash.audio;
        let audio_sampleRates = {
            '30280': '192000',
            '30232': '132000',
            '30216': '64000',
        };
        ...
        var video_str = video_str_list.join('\\n');
